...
int componentID = 0;
std::vector<int> component[10000];//最多有10000个强连通分量

struct Vertex
{
	bool output;
	int componentID;//连通分量id，用于最后输出
	int outdegree;//出度
...
int main()
{
	int n, m;
	std::cin >> n >> m;
	std::vector<int> dfn(n + 1, 0);//1-base数组
	std::vector<int> low(n + 1, 0);//1-base数组
	std::vector<bool> instack(n + 1, 0);//1-base数组
	std::vector<Vertex> vertices(n + 1);//1-base数组
	std::stack<int> stack;
	for (int i = 1;i <= m;++i)
	{
		int u, v;
